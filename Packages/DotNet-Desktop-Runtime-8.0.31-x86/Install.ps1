...
$InstallerName = 'windowsdesktop-runtime-8.0.31-win-x86.exe'
$InstallerPath = Join-Path $PSScriptRoot $InstallerName
$LogDirectory = 'C:\ProgramData\Microsoft\IntuneManagementExtension\Logs'
$LogPath = Join-Path $LogDirectory 'DotNet-Desktop-Runtime-8.0.31-x86-Install.log'
...
function Get-X86DotNetRuntimeEntries {
    $results = @()
    $baseKey = $null
    $uninstallKey = $null
    ...
    try {
        ...
        foreach ($subKeyName in $uninstallKey.GetSubKeyNames()) {
            ...
            try {
                ...
                if ($displayName -notmatch '^Microsoft (?<Family>Windows Desktop|\.NET) Runtime - (?<Version>8\.0\.\d+) \(x86\)') {
                    continue
                }
...
function Get-X86RuntimeInventory {
    $dotnetX86 = Join-Path ${env:ProgramFiles(x86)} 'dotnet\dotnet.exe'
    if (-not (Test-Path -LiteralPath $dotnetX86)) { return @() }
    return @(& $dotnetX86 --list-runtimes 2>$null)
}

try {
    Write-Log 'Starting Microsoft .NET Desktop Runtime 8.0.31 x86 deployment.'
    ...
    # Remove only Microsoft .NET 8.0.x x86 Desktop/Base Runtime patches older than 8.0.31.
    # Desktop entries are removed first, followed by any remaining base-runtime entries.
    $olderEntries = @(Get-X86DotNetRuntimeEntries | Where-Object {
        $_.Version.Major -eq 8 -and $_.Version.Minor -eq 0 -and $_.Version -lt $TargetVersion
    } | Sort-Object @{ Expression = { if ($_.Family -eq 'Windows Desktop') { 0 } else { 1 } } }, Version)

    if ($olderEntries.Count -eq 0) {
        Write-Log 'No older .NET 8.0.x x86 Desktop/Base Runtime entries were found.'
    }
    ...
    foreach ($entry in $olderEntries) {
        ...
        $stillInstalled = @(Get-X86DotNetRuntimeEntries | Where-Object { $_.KeyName -eq $entry.KeyName })
        if ($stillInstalled.Count -eq 0) {
            Write-Log "Skipping an entry already removed by a previous bundle: $($entry.DisplayName)"
            continue
        }

        Write-Log "Removing older x86 runtime: $($entry.DisplayName)"
        Invoke-RegisteredUninstall -Entry $stillInstalled[0]
    }

    $inventory = Get-X86RuntimeInventory
    ...
    if ($TargetVersion -notin $desktopVersions) { throw 'Microsoft.WindowsDesktop.App 8.0.31 x86 was not detected after installation.' }
    if ($TargetVersion -notin $baseVersions) { throw 'Microsoft.NETCore.App 8.0.31 x86 was not detected after installation.' }
    ...
    if ($olderDesktop.Count -gt 0 -or $olderBase.Count -gt 0) {
        throw "Older .NET 8.0.x x86 runtimes remain. Desktop: $($olderDesktop -join ', '); Base: $($olderBase -join ', ')"
    }

    Write-Log 'Deployment verification succeeded. Desktop and base Runtime 8.0.31 x86 are installed; older 8.0.x x86 patches are absent.'
    if ($RebootRequired) {
