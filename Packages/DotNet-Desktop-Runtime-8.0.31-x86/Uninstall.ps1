...
$InstallerPath = Join-Path $PSScriptRoot 'windowsdesktop-runtime-8.0.31-win-x86.exe'
$LogDirectory = 'C:\ProgramData\Microsoft\IntuneManagementExtension\Logs'
$LogPath = Join-Path $LogDirectory 'DotNet-Desktop-Runtime-8.0.31-x86-Uninstall.log'
    ...
    try {
        ...
        foreach ($subKeyName in $uninstallKey.GetSubKeyNames()) {
            ...
            try {
                ...
                if ($displayName -match '^Microsoft (?<Family>Windows Desktop|\.NET) Runtime - (?<Version>8\.0\.31) \(x86\)') {
...
try {
    Write-Log 'Starting removal of Microsoft .NET Desktop Runtime 8.0.31 x86.'
