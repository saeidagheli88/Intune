...
Write-Host 'Microsoft .NET Desktop Runtime 10.0.12 x64 compliance test'
Write-Host '----------------------------------------------------------'
...
$problems = @()
if ($TargetVersion -notin $desktopVersions) { $problems += 'Microsoft.WindowsDesktop.App 10.0.12 x64 is missing.' }
if ($TargetVersion -notin $baseVersions) { $problems += 'Microsoft.NETCore.App 10.0.12 x64 is missing.' }
...
Write-Host ''
Write-Host 'PASS: Required x64 Desktop/Base Runtime 10.0.12 are installed and older 10.0.x x64 patches are absent.' -ForegroundColor Green
exit 0
