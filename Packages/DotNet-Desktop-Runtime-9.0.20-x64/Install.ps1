...
$InstallerName = 'windowsdesktop-runtime-9.0.20-win-x64.exe'
$InstallerPath = Join-Path $PSScriptRoot $InstallerName
$LogDirectory = 'C:\ProgramData\Microsoft\IntuneManagementExtension\Logs'
$LogPath = Join-Path $LogDirectory 'DotNet-Desktop-Runtime-9.0.20-x64-Install.log'
    ...
    try {
        ...
        foreach ($subKeyName in $uninstallKey.GetSubKeyNames()) {
            ...
            try {
                ...
                if ($displayName -notmatch '^Microsoft (?<Family>Windows Desktop|\.NET) Runtime - (?<Version>9\.0\.\d+) \(x64\)') {
                    continue
                }
...
try {
    Write-Log 'Starting Microsoft .NET Desktop Runtime 9.0.20 x64 deployment.'
    ...
    # Remove only Microsoft .NET 9.0.x x64 Desktop/Base Runtime patches older than 9.0.20.
    ...
    $olderEntries = @(Get-X64DotNetRuntimeEntries | Where-Object {
        $_.Version.Major -eq 9 -and $_.Version.Minor -eq 0 -and $_.Version -lt $TargetVersion
    } | Sort-Object @{ Expression = { if ($_.Family -eq 'Windows Desktop') { 0 } else { 1 } } }, Version)

    if ($olderEntries.Count -eq 0) {
        Write-Log 'No older .NET 9.0.x x64 Desktop/Base Runtime entries were found.'
    }
    ...
    if ($TargetVersion -notin $desktopVersions) { throw 'Microsoft.WindowsDesktop.App 9.0.20 x64 was not detected after installation.' }
    if ($TargetVersion -notin $baseVersions) { throw 'Microsoft.NETCore.App 9.0.20 x64 was not detected after installation.' }
    ...
    if ($olderDesktop.Count -gt 0 -or $olderBase.Count -gt 0) {
        throw "Older .NET 9.0.x x64 runtimes remain. Desktop: $($olderDesktop -join ', '); Base: $($olderBase -join ', ')"
    }

    Write-Log 'Deployment verification succeeded. Desktop and base Runtime 9.0.20 x64 are installed; older 9.0.x x64 patches are absent.'
    if ($RebootRequired) {
