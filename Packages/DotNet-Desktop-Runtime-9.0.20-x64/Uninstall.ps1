...
$InstallerPath = Join-Path $PSScriptRoot 'windowsdesktop-runtime-9.0.20-win-x64.exe'
$LogDirectory = 'C:\ProgramData\Microsoft\IntuneManagementExtension\Logs'
$LogPath = Join-Path $LogDirectory 'DotNet-Desktop-Runtime-9.0.20-x64-Uninstall.log'
    ...
    try {
        ...
        foreach ($subKeyName in $uninstallKey.GetSubKeyNames()) {
            ...
            try {
                ...
                if ($displayName -match '^Microsoft (?<Family>Windows Desktop|\.NET) Runtime - (?<Version>9\.0\.20) \(x64\)') {
...
try {
    Write-Log 'Starting removal of Microsoft .NET Desktop Runtime 9.0.20 x64.'
