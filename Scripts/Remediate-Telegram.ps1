<#
.SYNOPSIS
    Removes ALL Telegram clients from a Windows device -- management decision
    is FULL REMOVAL (no Telegram client is allowed to remain). Pairs with
    Detect-Telegram.ps1 (keep the CONFIG block identical in both).

.DESCRIPTION
    Intune Remediation REMEDIATION script.

    WHAT IT DOES:
      1. Stops every running Telegram process (named Telegram.exe plus anything
         executing out of a 'Telegram Desktop' folder, which catches Updater.exe)
         so uninstalls/deletes cannot hang.
      2. Removes every machine-wide Telegram ARP entry found in the HKLM
         Uninstall keys (both registry views). Telegram Desktop is an Inno
         Setup install (no MSI code) -- entries are removed BRUTE-FORCE:
         registry key deleted, install folders handled by the folder pass.
         (An MSI-coded entry, should one ever appear, goes through msiexec /x
         with the 0/1605/3010/1641 success convention.) Never uses Win32_Product.
      3. Deletes the machine-wide 'Telegram Desktop' folders. A locked file is
      ...
      4. Sweeps EVERY user profile (loaded HKU hives in place; logged-off
         NTUSER.DAT hives 'reg load'ed and ALWAYS unloaded in finally):
           - deletes Telegram per-user Uninstall keys (per-user Inno installs
             cannot run their uninstaller reliably from SYSTEM -- brute-force
             is the reliable path and matches the folder deletion),
           - deletes Telegram Run values,
           - deletes the per-profile app folders (Roaming\Telegram Desktop is
             the DEFAULT per-user install location; tdata chat cache lives
             inside it, so full removal includes it),
           - deletes leftover Store package data folders
             (AppData\Local\Packages\TelegramMessengerLLP.*) opportunistically,
           - deletes tg:// protocol / tdesktop Classes keys,
           - deletes Telegram shortcuts (Desktop / Start Menu / Startup).
      5. Removes machine-level leftovers: HKLM Run values (both views), tg://
         protocol Classes keys, Start Menu and Public Desktop shortcuts.
      6. Removes the Telegram MSIX/Store package for all users and
         deprovisions it so new profiles don't get it back.
      7. VERIFY pass using the IDENTICAL classification rules as
         Detect-Telegram.ps1 (ARP entries + binary-gated folders with the
    ...
    EXIT CODES:
        ...
        exit 1 = at least one action FAILED / residual Telegram remains.
...
#--------------------------------------------------------------------------------
# Leftover Store package DATA folders under each profile's AppData\Local\Packages
# (wildcards allowed) -- cleaned opportunistically, never a finding.
$UserPackageDirPatterns = @('TelegramMessengerLLP.TelegramDesktop*')
# Leftover vendor registry keys (relative to SOFTWARE), machine and per-user.
$RegLeftoverNames = @('Telegram Desktop', 'TelegramDesktop', 'Telegram')
# Classes/protocol-handler keys: tg (protocol) and tdesktop.* ProgIDs. The
# (\.|$) tail keeps e.g. a hypothetical 'tgz' ProgID safe.
$TelegramClassesKeyRegex = '(?i)^(tg|tdesktop)(\.|$)'
# Processes to stop before touching anything (names without .exe).
$TelegramProcesses = @('Telegram')
# Path-based process kill: catches Updater.exe (too generic to kill by name).
$TelegramProcPathRegex = '(?i)\\Telegram Desktop\\'
# Regex for Run values / shortcuts that belong to Telegram.
$TelegramPathRegex = '(?i)telegram'
...
$LogFile = Join-Path $LogDir ("Remediate-Telegram_{0:yyyyMMdd_HHmmss}.log" -f (Get-Date))
...
# (Telegram Desktop ships as Inno Setup, but an MSI-coded entry is handled
# properly should one ever appear.)
function Uninstall-Msi {
    param([string]$ProductCode, [string]$Name)
    Write-Log "Uninstalling '$Name' ($ProductCode) via msiexec..."
    $msiLog = Join-Path $LogDir ("MsiUninstall-Telegram_{0}_{1:yyyyMMdd_HHmmss}.log" -f ($ProductCode -replace '[{}]',''), (Get-Date))
...
# Remove Telegram-referencing values from a Run key (HKLM views and HKU).
function Remove-TelegramRunValues {
    ...
    foreach ($valName in @($RunKey.GetValueNames())) {
        try {
            ...
            if (($valName -match $TelegramPathRegex) -or ($data -match $TelegramPathRegex)) {
...
# Telegram Classes/protocol keys (tg:, tdesktop.*) under an opened writable
# Classes key. Cleanup only -- never detection findings.
function Remove-TelegramLeftoverKeys {
...
function Remove-TelegramClassesKeys {
    ...
    foreach ($keyName in @($ClassesKey.GetSubKeyNames())) {
        if ($keyName -match $TelegramClassesKeyRegex) {
            try {
...
# Delete Telegram shortcuts (.lnk/.url) under a folder, recursively.
function Remove-TelegramShortcuts {
    ...
    try {
        Get-ChildItem -LiteralPath $Folder -Recurse -Force -File -ErrorAction SilentlyContinue |
            Where-Object { $_.Extension -in '.lnk','.url' -and $_.BaseName -match $TelegramPathRegex } |
            ForEach-Object {
                try {
        ...
        Get-ChildItem -LiteralPath $Folder -Recurse -Force -Directory -ErrorAction SilentlyContinue |
            Where-Object { $_.Name -match $TelegramPathRegex } |
            Sort-Object { $_.FullName.Length } -Descending | ForEach-Object {
...
Write-Log "=== Telegram removal remediation started on $env:COMPUTERNAME (64-bit host: $([Environment]::Is64BitProcess)) ==="
...
try {
    ...
    #----------------------------------------------------------------------------
    # 2. Stop every Telegram process (user sessions included) so nothing blocks.
    #    Named processes first, then a PATH-based pass that catches Updater.exe
    #    (its name is far too generic to kill blindly).
    #----------------------------------------------------------------------------
    foreach ($procName in $TelegramProcesses) {
        try {
    ...
    try {
        Get-Process -ErrorAction SilentlyContinue | Where-Object {
            $_.Path -and ($_.Path -match $TelegramProcPathRegex)
        } | ForEach-Object {
            try {
    ...
    #----------------------------------------------------------------------------
    # 3. Machine-wide ARP entries (both views): MSI -> msiexec; non-MSI (the
    #    normal Inno case) -> brute-force key deletion (folders in step 4).
    #----------------------------------------------------------------------------
    ...
    foreach ($app in $machineEntries) {
        ...
        if ($code) {
            ...
            # Non-MSI (Inno Setup) registration -- remove the ARP key here and
            # let the folder pass (step 4) take the files.
    ...
    if ($machineEntries.Count -eq 0) { Write-Log 'No machine-wide Telegram ARP entries found.' }

    #----------------------------------------------------------------------------
    # 4. Machine-wide folders (wholly Telegram-owned).
    #----------------------------------------------------------------------------
    ...
    #----------------------------------------------------------------------------
    # 5. Machine-level leftovers: HKLM Run values, vendor keys, tg:// Classes
    ...
    foreach ($view in $hklmViews) {
        ...
        try {
            ...
            if ($run) { Remove-TelegramRunValues -RunKey $run -Scope "HKLM $view"; $run.Close() }
            $sw = $base.OpenSubKey('SOFTWARE', $true)
            if ($sw) { Remove-TelegramLeftoverKeys -SoftwareKey $sw -Scope "HKLM $view"; $sw.Close() }
            $classes = $base.OpenSubKey('SOFTWARE\Classes', $true)
            if ($classes) { Remove-TelegramClassesKeys -ClassesKey $classes -Scope "HKLM $view"; $classes.Close() }
        } catch { Write-Log "HKLM leftover sweep error ($view): $($_.Exception.Message)" 'WARN' }
        finally { if ($base) { $base.Close() } }
    }

    Remove-TelegramShortcuts -Folder (Join-Path $env:ProgramData 'Microsoft\Windows\Start Menu\Programs') -Scope 'AllUsers StartMenu'
    Remove-TelegramShortcuts -Folder (Join-Path $env:PUBLIC 'Desktop') -Scope 'Public Desktop'

    #----------------------------------------------------------------------------
    # 6. Per-user sweep: uninstall keys, Run values, app folders, Store package
    #    data, shortcuts -- for EVERY real profile (offline hives loaded/unloaded).
    #----------------------------------------------------------------------------
    ...
    function Remove-UserTelegramRegistry {
        ...
        try {
            ...
            if ($run) { Remove-TelegramRunValues -RunKey $run -Scope $Scope; $run.Close() }
            $sw = $users.OpenSubKey("$relRoot\SOFTWARE", $true)
            if ($sw) { Remove-TelegramLeftoverKeys -SoftwareKey $sw -Scope $Scope; $sw.Close() }
            $classes = $users.OpenSubKey("$relRoot\SOFTWARE\Classes", $true)
            if ($classes) { Remove-TelegramClassesKeys -ClassesKey $classes -Scope "$Scope Classes"; $classes.Close() }
            $users.Close()
    ...
    foreach ($prof in $profiles) {
        ...
        try {
            ...
            if ($hiveAvailable) {
                Remove-UserTelegramRegistry -HiveRoot $hiveRoot -Scope "HKU:$sid"
            }
            ...
            if (-not [string]::IsNullOrWhiteSpace($profPath) -and (Test-Path -LiteralPath $profPath)) {
                ...
                Remove-TelegramShortcuts -Folder (Join-Path $profPath 'Desktop') -Scope "Desktop:$sid"
                Remove-TelegramShortcuts -Folder (Join-Path $profPath 'AppData\Roaming\Microsoft\Windows\Start Menu\Programs') -Scope "StartMenu:$sid"
            }
        } catch {
    ...
    #----------------------------------------------------------------------------
    # 6b. MSIX/Store Telegram packages. Removed for all users + deprovisioned
    #     so new profiles don't get them back.
    #----------------------------------------------------------------------------
    ...
    #----------------------------------------------------------------------------
    # 7. VERIFY -- identical classification rules as Detect-Telegram.ps1 (ARP
    ...
    function Test-LiveTelegramBinary {
        param([string]$Dir)
    ...
    foreach ($dir in ($MachineDirs | Select-Object -Unique)) {
        if (Test-LiveTelegramBinary -Dir $dir) { $residual.Add("folder: $dir") }
    }
    foreach ($prof in $profiles) {
        $profPath = $prof.ProfileImagePath
        ...
        foreach ($rel in $UserAppDirRel) {
            $dir = Join-Path $profPath $rel
            if (Test-LiveTelegramBinary -Dir $dir) { $residual.Add("user folder: $dir") }
        }
    }
    ...
    if ($residual.Count -eq 0) { Write-Log 'VERIFY: no residual Telegram found.' }
    ...
    if ($failures.Count -gt 0) {
        Write-Log "RESULT: Telegram removal completed WITH FAILURES." 'ERROR'
        try { Stop-Transcript -ErrorAction SilentlyContinue | Out-Null } catch { }
        Write-Output "Telegram removal incomplete: $summary"
        exit 1
    } else {
        Write-Log 'RESULT: Telegram removal completed successfully.'
        try { Stop-Transcript -ErrorAction SilentlyContinue | Out-Null } catch { }
        Write-Output "Telegram removal OK: $summary"
        exit 0
    }
}
catch {
    ...
    try { Stop-Transcript -ErrorAction SilentlyContinue | Out-Null } catch { }
    Write-Output "Telegram remediation error: $($_.Exception.Message)"
    exit 1
}
