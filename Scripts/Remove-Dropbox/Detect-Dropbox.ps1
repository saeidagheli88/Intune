<#
.SYNOPSIS
    Detects ANY Dropbox desktop app install on a Windows device -- management
    decision is FULL REMOVAL (no Dropbox app is allowed to remain). Pairs
    with Remediate-Dropbox.ps1 (keep the CONFIG block identical in both).

.DESCRIPTION
    Intune Remediation DETECTION script (READ-ONLY).

    SCOPE -- everything Dropbox APP counts as a finding, machine-wide AND
    per-user:
      - The classic desktop app: default install is PER-USER in
        <profile>\AppData\Local\Dropbox (Client\Dropbox.exe) with an HKU
        uninstall key; the enterprise/offline installer can also install
        machine-wide to Program Files (x86)\Dropbox\Client with an HKLM key.
      - "Dropbox Update Helper" (Omaha-fork updater) in
        Program Files (x86)\Dropbox\Update -- its DropboxUpdate.exe binary
        gates a folder finding too, since a surviving updater can put the
        app back. Its services (dbupdate/dbupdatem) and DbxSvc are removed
        by remediation (leftover services are never detection findings).
      - Any Dropbox MSIX/Store package (C27EB4BA.Dropbox), any user or
        provisioned.

    *** THE USER'S SYNCED FILES ARE NEVER TOUCHED. The synced-content folder
    (<profile>\Dropbox, or a custom location) is DATA, not the app -- it is
    not scanned, not a finding, and remediation never deletes it. Only the
    app folders (AppData\Local\Dropbox, Program Files (x86)\Dropbox) and
    settings (AppData\Roaming\Dropbox) are in scope. ***

    WHAT COUNTS AS A FINDING (exit 1):
      1. Any HKLM Uninstall entry (BOTH 64-bit and 32-bit registry views)
         classified as Dropbox by Test-IsDropbox (includes "Dropbox Update
         Helper").
      2. Any per-user (HKU) Uninstall entry classified as Dropbox -- every
         real profile is swept; logged-off NTUSER.DAT hives are temporarily
         'reg load'ed and ALWAYS unloaded in finally.
      3. A Dropbox BINARY (Dropbox.exe / DropboxUpdate.exe / DbxSvc.exe)
         still on disk in the known machine or per-profile APP folders.
         Folder-only leftovers (no binary) are NOT findings, and a binary
         already queued in PendingFileRenameOperations (delete-on-reboot) is
         NOT a finding -- loop-safety, same convention as the other pairs.
      4. Any Dropbox MSIX package (any user or provisioned).

    NOT findings (remediation cleans them opportunistically, but they must not
    flap detection): AppData\Roaming\Dropbox settings, shortcuts, Run values,
    Dropbox Classes keys, shell icon overlay registrations, leftover
    services/scheduled tasks.

    CLASSIFIER SAFETY: the strong regex anchors the word 'dropbox' at the
    START of the DisplayName ('Dropbox', 'Dropbox Update Helper', ...); the
    loose regex requires a Dropbox publisher. Appx matching excludes
...
#--------------------------------------------------------------------------------
# Strong name match: DisplayName starting with 'Dropbox' ('Dropbox',
# 'Dropbox Update Helper'). Enough on its own (publisher may be blank on HKU
# keys).
$DbxStrongName  = '(?i)^\s*dropbox\b'
# Loose match: 'dropbox' anywhere in the name, but ONLY with a Dropbox publisher.
$DbxLooseName   = '(?i)\bdropbox\b'
# Binaries that gate a folder finding (folder-only leftovers are not findings).
# DropboxUpdate.exe counts: a surviving updater can reinstall the app.
$DbxBinaryNames = @('Dropbox.exe','DropboxUpdate.exe','DbxSvc.exe')
# Machine-wide APP folders (the vendor dir holds only Client + Update; the
# user's synced files never live here).
$MachineDirs = @(
...
# Per-profile APP folders (findings when a binary is inside). This is the
# APP install dir only -- NEVER the synced-content folder <profile>\Dropbox.
...
# MSIX/Store packages: name containing 'dropbox' (the Store package is
# C27EB4BA.Dropbox), never Microsoft-published.
...
$LogFile = Join-Path $LogDir ("Detect-Dropbox_{0:yyyyMMdd_HHmmss}.log" -f (Get-Date))
...
Write-Log "=== Dropbox removal detection started on $env:COMPUTERNAME (64-bit host: $([Environment]::Is64BitProcess)) ==="
...
try {
    ...
    #----------------------------------------------------------------------------
    # 4. File-system check: machine APP folders + per-profile APP folders.
    #    Binary-gated (a folder with no Dropbox binary is NOT a finding) and
    ...
    #    The synced-content folder <profile>\Dropbox is NEVER scanned.
    #----------------------------------------------------------------------------
    ...
    # Returns Dropbox binaries under $Dir that are NOT queued for delete-on-reboot.
    function Get-LiveDropboxBinary {
        param([string]$Dir)
        $live = @()
    ...
    foreach ($dir in ($MachineDirs | Select-Object -Unique)) {
        $bins = @(Get-LiveDropboxBinary -Dir $dir)
        if ($bins.Count -gt 0) {
            Add-Finding ("[FileSystem] {0} ({1} Dropbox binaries, e.g. {2})" -f $dir, $bins.Count, (Split-Path $bins[0] -Leaf))
        }
    }
    foreach ($prof in $profiles) {
        $profPath = $prof.ProfileImagePath
        ...
        foreach ($rel in $UserAppDirRel) {
            ...
            $bins = @(Get-LiveDropboxBinary -Dir $dir)
            if ($bins.Count -gt 0) {
                Add-Finding ("[FileSystem user] {0} ({1} Dropbox binaries)" -f $dir, $bins.Count)
            }
        }
    }

    #----------------------------------------------------------------------------
    # 4b. MSIX/Store Dropbox packages (C27EB4BA.Dropbox), any user + provisioned.
    #----------------------------------------------------------------------------
    ...
    if ($found) {
        ...
        Write-Log "RESULT: Dropbox removal NEEDED ($($details.Count) finding(s))" 'WARN'
        try { Stop-Transcript -ErrorAction SilentlyContinue | Out-Null } catch { }
        Write-Output "Dropbox removal needed: $summary"
        exit 1
    } else {
        Write-Log 'RESULT: no Dropbox app found. Device is clean.'
        try { Stop-Transcript -ErrorAction SilentlyContinue | Out-Null } catch { }
        Write-Output 'Dropbox clean: no Dropbox app installs found (machine or per-user).'
        exit 0
    }
}
catch {
    ...
    try { Stop-Transcript -ErrorAction SilentlyContinue | Out-Null } catch { }
    Write-Output "Dropbox detection error (treating as found): $($_.Exception.Message)"
    exit 1
}
