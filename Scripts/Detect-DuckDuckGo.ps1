<#
.SYNOPSIS
    Detects ANY DuckDuckGo Browser install on a Windows device -- management
    decision is FULL REMOVAL (no DuckDuckGo browser is allowed to remain).
    Pairs with Remediate-DuckDuckGo.ps1 (keep the CONFIG block identical).

.DESCRIPTION
    Intune Remediation DETECTION script (READ-ONLY).

    SCOPE -- everything DuckDuckGo counts as a finding, machine-wide AND
    per-user. The DuckDuckGo browser for Windows ships PRIMARILY as an
    MSIX/Store package (DuckDuckGo.DesktopBrowser) -- even the website's exe
    installer registers the MSIX package -- so the Appx checks are the main
    signal; classic ARP/folder checks are the backstop for older or unusual
    installs:
      - DuckDuckGo MSIX package for ANY user, plus provisioned copies that
        would come back on new profiles.
      - Any HKLM/HKU Uninstall (ARP) entry classified as DuckDuckGo.
      - DuckDuckGo.exe on disk in the known machine or per-profile install
        folders.

    WHAT COUNTS AS A FINDING (exit 1):
      1. Any HKLM Uninstall entry (BOTH 64-bit and 32-bit registry views)
         classified as DuckDuckGo by Test-IsDuck.
      2. Any per-user (HKU) Uninstall entry classified as DuckDuckGo -- every
         real profile is swept; logged-off NTUSER.DAT hives are temporarily
         'reg load'ed and ALWAYS unloaded in finally.
      3. A DuckDuckGo BINARY (DuckDuckGo.exe) still on disk in the known
         machine or per-profile install folders. Folder-only leftovers (no
         binary) are NOT findings, and a binary already queued in
         PendingFileRenameOperations (delete-on-reboot) is NOT a finding --
         loop-safety, same convention as the 7-Zip/Adobe/Webex pairs.
      4. Any DuckDuckGo MSIX package (installed for any user, or provisioned).

    NOT findings (remediation cleans them opportunistically, but they must not
    flap detection): browser data folders (AppData\Local\DuckDuckGo,
    AppData\Local\Packages\DuckDuckGo.*), shortcuts, Run values, Classes keys.

    CLASSIFIER SAFETY: 'duckduckgo' is a globally unique brand string; the
    Appx check still excludes Microsoft publishers as a safety net.
...
#--------------------------------------------------------------------------------
# Strong name match: enough on its own ('duckduckgo' is globally unique).
...
# Machine-wide install folders (all wholly DuckDuckGo-owned).
$MachineDirs = @(
...
# MSIX/Store packages: name containing 'duckduckgo' (official package is
# DuckDuckGo.DesktopBrowser). Microsoft publishers excluded as a safety net.
...
$LogFile = Join-Path $LogDir ("Detect-DuckDuckGo_{0:yyyyMMdd_HHmmss}.log" -f (Get-Date))
...
Write-Log "=== DuckDuckGo removal detection started on $env:COMPUTERNAME (64-bit host: $([Environment]::Is64BitProcess)) ==="
...
try {
    ...
    #    Binary-gated (a folder with no DuckDuckGo binary is NOT a finding) and
    ...
    # Returns DuckDuckGo binaries under $Dir that are NOT queued for delete-on-reboot.
    function Get-LiveDuckBinary {
        param([string]$Dir)
        $live = @()
    ...
    foreach ($dir in ($MachineDirs | Select-Object -Unique)) {
        $bins = @(Get-LiveDuckBinary -Dir $dir)
        if ($bins.Count -gt 0) {
            Add-Finding ("[FileSystem] {0} ({1} DuckDuckGo binaries, e.g. {2})" -f $dir, $bins.Count, (Split-Path $bins[0] -Leaf))
        }
    }
    foreach ($prof in $profiles) {
        $profPath = $prof.ProfileImagePath
        ...
        foreach ($rel in $UserAppDirRel) {
            ...
            $bins = @(Get-LiveDuckBinary -Dir $dir)
            if ($bins.Count -gt 0) {
                Add-Finding ("[FileSystem user] {0} ({1} DuckDuckGo binaries)" -f $dir, $bins.Count)
            }
        }
    }

    #----------------------------------------------------------------------------
    # 4b. MSIX/Store DuckDuckGo packages -- the PRIMARY install vector for this
    #     browser. Any user + provisioned copies.
    #----------------------------------------------------------------------------
    ...
    if ($found) {
        ...
        Write-Log "RESULT: DuckDuckGo removal NEEDED ($($details.Count) finding(s))" 'WARN'
        try { Stop-Transcript -ErrorAction SilentlyContinue | Out-Null } catch { }
        Write-Output "DuckDuckGo removal needed: $summary"
        exit 1
    } else {
        Write-Log 'RESULT: no DuckDuckGo browser found. Device is clean.'
        try { Stop-Transcript -ErrorAction SilentlyContinue | Out-Null } catch { }
        Write-Output 'DuckDuckGo clean: no DuckDuckGo browser installed (machine, per-user, or Store).'
        exit 0
    }
}
catch {
    ...
    try { Stop-Transcript -ErrorAction SilentlyContinue | Out-Null } catch { }
    Write-Output "DuckDuckGo detection error (treating as found): $($_.Exception.Message)"
    exit 1
}
