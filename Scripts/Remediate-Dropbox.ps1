<#
.SYNOPSIS
    Removes ALL Dropbox desktop app installs from a Windows device --
    management decision is FULL REMOVAL (no Dropbox app is allowed to
    remain). Pairs with Detect-Dropbox.ps1 (keep the CONFIG block identical
    in both).

.DESCRIPTION
    Intune Remediation REMEDIATION script.

    *** THE USER'S SYNCED FILES ARE NEVER TOUCHED. The synced-content folder
    (<profile>\Dropbox, or a custom location) is DATA, not the app -- this
    script never deletes it. Only the app (AppData\Local\Dropbox,
    Program Files (x86)\Dropbox), its settings (AppData\Roaming\Dropbox),
    updater, services, tasks and registry footprint are removed. Files
    already synced remain in the local folder AND in the cloud; anything
    not yet uploaded stays safely in the local folder. ***

    WHAT IT DOES:
      1. Stops every running Dropbox process (Dropbox.exe, DropboxUpdate.exe,
         DbxSvc, installers/uninstallers, plus anything executing out of the
         Dropbox APP folders) so uninstalls/deletes cannot hang.
      2. For MACHINE-WIDE installs, first tries the official uninstaller
         (Client\DropboxUninstaller.exe /S) with a bounded wait. Best-effort:
         exit 0 is logged as success, anything else is tolerated because the
         brute-force passes below guarantee the end state either way.
      3. Removes every machine-wide Dropbox ARP entry that survives (both
         registry views): MSI-coded entries via msiexec (0/1605/3010/1641
         convention, covers "Dropbox Update Helper"); everything else
         brute-force key deletion. Never uses Win32_Product.
      4. Stops and deletes Dropbox services -- dbupdate/dbupdatem (matched by
         BINARY PATH under \Dropbox\Update\, never name alone) and DbxSvc
         (System32\DbxSvc.exe) -- and unregisters Dropbox* scheduled tasks
         (DropboxUpdateTaskMachine*, per-user DropboxUpdateTaskUser*) so
         nothing can reinstall it. The orphaned System32\DbxSvc.exe binary
         is deleted too.
      5. Deletes the machine-wide Dropbox APP folders. A locked file is
      ...
      6. Sweeps EVERY user profile (loaded HKU hives in place; logged-off
         NTUSER.DAT hives 'reg load'ed and ALWAYS unloaded in finally):
           - deletes Dropbox per-user Uninstall keys (per-user installs
             cannot run their uninstaller reliably from SYSTEM -- brute-force
             is the reliable path and matches the folder deletion),
           - deletes Dropbox Run values (autostart),
           - deletes AppData\Local\Dropbox (the APP) and AppData\Roaming\
             Dropbox (settings) -- NEVER the synced-content folder,
           - deletes Dropbox Classes keys and Dropbox shortcuts
             (Desktop / Start Menu / Startup).
      7. Removes machine-level leftovers: HKLM Run values (both views),
         HKLM\SOFTWARE\Dropbox + DropboxUpdate vendor keys, Dropbox* Classes
         keys, DropboxExt shell icon overlay registrations (both views),
         Start Menu and Public Desktop shortcuts.
      8. Removes any Dropbox MSIX/Store package (C27EB4BA.Dropbox) for all
         users + deprovisions.
      9. VERIFY pass using the IDENTICAL classification rules as
         Detect-Dropbox.ps1 (ARP entries + binary-gated folders with the
         pending-delete guard + Appx). Any residual -> exit 1, so Intune
         shows the device honestly as "not fixed" instead of flapping.

    IDEMPOTENT: second run finds nothing -> no-op exit 0.

    EXIT CODES:
        exit 0 = success or no-op (including "done, reboot pending").
        exit 1 = at least one action FAILED / residual Dropbox remains.
...
#--------------------------------------------------------------------------------
# CONFIG -- keep IDENTICAL in Detect-Dropbox.ps1 / Remediate-Dropbox.ps1
#--------------------------------------------------------------------------------
$DbxStrongName  = '(?i)^\s*dropbox\b'
$DbxLooseName   = '(?i)\bdropbox\b'
$DbxBinaryNames = @('Dropbox.exe','DropboxUpdate.exe','DbxSvc.exe')
$MachineDirs = @(
    (Join-Path $env:ProgramFiles 'Dropbox')
)
if (${env:ProgramFiles(x86)}) {
    $MachineDirs += (Join-Path ${env:ProgramFiles(x86)} 'Dropbox')
}
$UserAppDirRel = @('AppData\Local\Dropbox')

function Test-IsDropbox {
    param([string]$Name, [string]$Publisher)
    if ([string]::IsNullOrWhiteSpace($Name)) { return $false }
    if ($Name -match $DbxStrongName) { return $true }
    return (($Publisher -match '(?i)dropbox') -and ($Name -match $DbxLooseName))
}

function Test-IsDropboxAppx {
    param([string]$Name, [string]$Publisher)
    if ([string]::IsNullOrWhiteSpace($Name)) { return $false }
    if ($Publisher -match '(?i)CN=Microsoft') { return $false }
    return ($Name -match '(?i)dropbox')
}

#--------------------------------------------------------------------------------
# Remediation-only config (NOT part of the shared classifier).
#--------------------------------------------------------------------------------
# Per-profile DATA folders: settings/metadata only, cleaned opportunistically,
# never a finding. The SYNCED-CONTENT folder <profile>\Dropbox is deliberately
# NOT listed anywhere in this script and is never deleted.
$UserDataDirRel = @('AppData\Roaming\Dropbox')
# Leftover vendor registry keys (relative to SOFTWARE), machine and per-user.
$RegLeftoverNames = @('Dropbox','DropboxUpdate')
# Classes/protocol-handler keys (Dropbox.*, DropboxExt* ...).
$DbxClassesKeyRegex = '(?i)^dropbox'
# Processes to stop by name before touching anything (names without .exe).
$DbxProcesses = @('Dropbox','DropboxUpdate','DropboxInstaller','DropboxUninstaller','DbxSvc','DropboxCrashHandler')
# Path-based process kill: APP folders only (Client/Update) -- deliberately
# NOT a bare \Dropbox\ match, so nothing running from a user's synced-content
# folder <profile>\Dropbox is ever touched.
$DbxProcPathRegex = '(?i)\\Dropbox\\(Client|Update)\\'
# Services matched by BINARY PATH (never name alone): dbupdate/dbupdatem run
# from \Dropbox\Update\, DbxSvc from System32\DbxSvc.exe.
$DbxServicePathRegex = '(?i)(\\Dropbox\\(Client|Update)\\|\\DbxSvc\.exe)'
# Scheduled tasks created by Dropbox Update (machine + per-user).
$DbxTaskNameRegex = '(?i)^Dropbox'
# Regex for Run values / shortcuts that belong to Dropbox.
$DbxPathRegex = '(?i)dropbox'
...
$LogFile = Join-Path $LogDir ("Remediate-Dropbox_{0:yyyyMMdd_HHmmss}.log" -f (Get-Date))
...
# ("Dropbox Update Helper" registers under a GUID key; whether or not it is
# truly MSI-registered, 1605 is tolerated and the orphaned key is then
# deleted directly.)
function Uninstall-Msi {
    param([string]$ProductCode, [string]$Name)
    Write-Log "Uninstalling '$Name' ($ProductCode) via msiexec..."
    $msiLog = Join-Path $LogDir ("MsiUninstall-Dropbox_{0}_{1:yyyyMMdd_HHmmss}.log" -f ($ProductCode -replace '[{}]',''), (Get-Date))
...
# Best-effort official uninstall for MACHINE-WIDE installs:
# <VendorRoot>\Client\DropboxUninstaller.exe /S. Bounded wait so a wedged
# uninstaller can never hang the remediation; any non-zero outcome is
# tolerated -- the brute-force passes below guarantee the end state anyway.
function Invoke-DropboxSetupUninstall {
    param([string]$VendorRoot)   # e.g. C:\Program Files (x86)\Dropbox
    $ran = $false
    try {
        $uninst = Join-Path $VendorRoot 'Client\DropboxUninstaller.exe'
        if (Test-Path -LiteralPath $uninst) {
            $ran = $true
            Write-Log "Running official uninstaller: $uninst /S"
            try {
                $p = Start-Process -FilePath $uninst -ArgumentList '/S' -PassThru
                if (-not $p.WaitForExit(180000)) {
                    ...
                    $script:actions.Add("ran official Dropbox uninstaller ($VendorRoot)")
                } else {
                    Write-Log "Official uninstaller exit $($p.ExitCode) (tolerated; brute-force follows)." 'WARN'
                }
            } catch {
                Write-Log "Official uninstaller error ($uninst): $($_.Exception.Message) (tolerated)." 'WARN'
            }
        }
...
# Remove Dropbox-referencing values from a Run key (HKLM views and HKU).
function Remove-DropboxRunValues {
    ...
    foreach ($valName in @($RunKey.GetValueNames())) {
        try {
            ...
            if (($valName -match $DbxPathRegex) -or ($data -match $DbxPathRegex)) {
...
# Delete leftover vendor keys (Dropbox, DropboxUpdate) under an opened writable
# SOFTWARE key, plus Dropbox Classes keys under an opened writable Classes key.
# Cleanup only -- never detection findings.
function Remove-DropboxLeftoverKeys {
...
function Remove-DropboxClassesKeys {
    ...
    foreach ($keyName in @($ClassesKey.GetSubKeyNames())) {
        if ($keyName -match $DbxClassesKeyRegex) {
            try {
...
# Delete Dropbox shortcuts (.lnk/.url) under a folder, recursively. Link files
# only -- the synced-content folder itself is never under Desktop/Start Menu.
function Remove-DropboxShortcuts {
    ...
    try {
        Get-ChildItem -LiteralPath $Folder -Recurse -Force -File -ErrorAction SilentlyContinue |
            Where-Object { $_.Extension -in '.lnk','.url' -and $_.BaseName -match $DbxPathRegex } |
            ForEach-Object {
                try {
        ...
        Get-ChildItem -LiteralPath $Folder -Recurse -Force -Directory -ErrorAction SilentlyContinue |
            Where-Object { $_.Name -match $DbxPathRegex } |
            Sort-Object { $_.FullName.Length } -Descending | ForEach-Object {
...
Write-Log "=== Dropbox removal remediation started on $env:COMPUTERNAME (64-bit host: $([Environment]::Is64BitProcess)) ==="
...
try {
    ...
    #----------------------------------------------------------------------------
    # 2. Stop every Dropbox process (user sessions included) so nothing blocks.
    #    Named processes first, then a PATH-based pass restricted to the APP
    #    folders (Client/Update) -- never the synced-content folder.
    #----------------------------------------------------------------------------
    foreach ($procName in $DbxProcesses) {
        try {
    ...
    try {
        Get-Process -ErrorAction SilentlyContinue | Where-Object {
            $_.Path -and ($_.Path -match $DbxProcPathRegex)
        } | ForEach-Object {
            try {
    ...
    foreach ($dir in ($MachineDirs | Select-Object -Unique)) {
        if (Test-Path -LiteralPath $dir) { [void](Invoke-DropboxSetupUninstall -VendorRoot $dir) }
    }
        ...
        try {
            ...
            if ($uninstall) {
                foreach ($subName in $uninstall.GetSubKeyNames()) {
                    ...
                    if (Test-IsDropbox -Name $name -Publisher $pub) {
    ...
    foreach ($app in $machineEntries) {
        ...
        if ($code) {
            ...
            # delete the key directly -- same flap protection as the other pairs.
            $base = $null
            try {
                $base = [Microsoft.Win32.RegistryKey]::OpenBaseKey(
            ...
            # Non-MSI registration (the normal machine-wide Dropbox case when
            # the official uninstaller didn't take it) -- remove the ARP key
            # here and let the folder pass (step 5) take the files.
    ...
    if ($machineEntries.Count -eq 0) { Write-Log 'No machine-wide Dropbox ARP entries found (after official uninstall pass).' }

    #----------------------------------------------------------------------------
    # 4. Dropbox services and scheduled tasks (Dropbox Update would otherwise
    #    reinstall the app). Services matched by binary PATH, never name.
    #    Also deletes the orphaned System32\DbxSvc.exe binary.
    #----------------------------------------------------------------------------
    try {
        Get-CimInstance -ClassName Win32_Service -ErrorAction SilentlyContinue |
            Where-Object { $_.PathName -and ($_.PathName -match $DbxServicePathRegex) } |
            ForEach-Object {
                $svcName = $_.Name
                Write-Log "Dropbox service '$svcName' present (state $($_.State)); stopping and deleting." 'WARN'
    ...
    $dbxSvcExe = Join-Path $env:WINDIR 'System32\DbxSvc.exe'
    if (Test-Path -LiteralPath $dbxSvcExe) {
        try {
            Remove-Item -LiteralPath $dbxSvcExe -Force -ErrorAction Stop
            Write-Log "Removed orphaned service binary: $dbxSvcExe"
            $actions.Add('removed System32\DbxSvc.exe')
        } catch {
            Write-Log "DbxSvc.exe locked; queuing for delete-on-reboot." 'WARN'
            [void](Register-DeleteOnReboot -Path $dbxSvcExe)
        }
    }
    ...
    if (Get-Command -Name Get-ScheduledTask -ErrorAction SilentlyContinue) {
        try {
            Get-ScheduledTask -ErrorAction SilentlyContinue |
                Where-Object { $_.TaskName -match $DbxTaskNameRegex } |
                ForEach-Object {
                    try {
    ...
    #----------------------------------------------------------------------------
    # 5. Machine-wide APP folders (Program Files [x86]\Dropbox).
    #----------------------------------------------------------------------------
    foreach ($dir in ($MachineDirs | Select-Object -Unique)) {
    ...
    #----------------------------------------------------------------------------
    # 6. Machine-level leftovers: HKLM Run values, Dropbox vendor keys,
    #    Dropbox* Classes keys, DropboxExt shell icon overlays (both views),
    ...
    foreach ($view in $hklmViews) {
        ...
        try {
            ...
            if ($run) { Remove-DropboxRunValues -RunKey $run -Scope "HKLM $view"; $run.Close() }
            $sw = $base.OpenSubKey('SOFTWARE', $true)
            if ($sw) { Remove-DropboxLeftoverKeys -SoftwareKey $sw -Scope "HKLM $view"; $sw.Close() }
            $classes = $base.OpenSubKey('SOFTWARE\Classes', $true)
            if ($classes) { Remove-DropboxClassesKeys -ClassesKey $classes -Scope "HKLM $view"; $classes.Close() }
            # Explorer shell icon overlay registrations (" DropboxExt01" etc. --
            # names often carry a leading space to sort first).
            try {
                $ovl = $base.OpenSubKey('SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\ShellIconOverlayIdentifiers', $true)
                if ($ovl) {
                    foreach ($kn in @($ovl.GetSubKeyNames())) {
                        if ($kn -match '(?i)dropbox') {
                            $ovl.DeleteSubKeyTree($kn, $false)
                            Write-Log "Removed shell icon overlay '$kn' [HKLM $view]."
                            $actions.Add("removed icon overlay $($kn.Trim()) [$view]")
                        }
                    }
                    $ovl.Close()
                }
            } catch { Write-Log "Icon overlay cleanup error ($view): $($_.Exception.Message)" 'WARN' }
        } catch { Write-Log "HKLM leftover sweep error ($view): $($_.Exception.Message)" 'WARN' }
        finally { if ($base) { $base.Close() } }
    }

    Remove-DropboxShortcuts -Folder (Join-Path $env:ProgramData 'Microsoft\Windows\Start Menu\Programs') -Scope 'AllUsers StartMenu'
    Remove-DropboxShortcuts -Folder (Join-Path $env:PUBLIC 'Desktop') -Scope 'Public Desktop'

    #----------------------------------------------------------------------------
    # 7. Per-user sweep: uninstall keys, Run values, app + settings folders,
    #    shortcuts -- for EVERY real profile (offline hives loaded/unloaded).
    #    The synced-content folder <profile>\Dropbox is NEVER touched.
    #----------------------------------------------------------------------------
    ...
    function Remove-UserDropboxRegistry {
        ...
        foreach ($node in @('SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall',
            ...
            try {
                ...
                Get-ChildItem -LiteralPath $path -ErrorAction SilentlyContinue | ForEach-Object {
                    ...
                    if ($p -and (Test-IsDropbox -Name ([string]$p.DisplayName) -Publisher ([string]$p.Publisher))) {
                        try {
        ...
        try {
            ...
            if ($run) { Remove-DropboxRunValues -RunKey $run -Scope $Scope; $run.Close() }
            $sw = $users.OpenSubKey("$relRoot\SOFTWARE", $true)
            if ($sw) { Remove-DropboxLeftoverKeys -SoftwareKey $sw -Scope $Scope; $sw.Close() }
            $classes = $users.OpenSubKey("$relRoot\SOFTWARE\Classes", $true)
            if ($classes) { Remove-DropboxClassesKeys -ClassesKey $classes -Scope "$Scope Classes"; $classes.Close() }
            $users.Close()
    ...
    foreach ($prof in $profiles) {
        ...
        try {
            ...
            if ($hiveAvailable) {
                Remove-UserDropboxRegistry -HiveRoot $hiveRoot -Scope "HKU:$sid"
            }
            ...
            if (-not [string]::IsNullOrWhiteSpace($profPath) -and (Test-Path -LiteralPath $profPath)) {
                ...
                Remove-DropboxShortcuts -Folder (Join-Path $profPath 'Desktop') -Scope "Desktop:$sid"
                Remove-DropboxShortcuts -Folder (Join-Path $profPath 'AppData\Roaming\Microsoft\Windows\Start Menu\Programs') -Scope "StartMenu:$sid"
            }
        } catch {
    ...
    #----------------------------------------------------------------------------
    # 7b. MSIX/Store Dropbox packages. Removed for all users + deprovisioned.
    #----------------------------------------------------------------------------
    if (Get-Command -Name Get-AppxPackage -ErrorAction SilentlyContinue) {
        try {
            $pkgs = Get-AppxPackage -AllUsers -ErrorAction SilentlyContinue |
                    Where-Object { Test-IsDropboxAppx -Name $_.Name -Publisher $_.Publisher }
            foreach ($pkg in @($pkgs)) {
                try {
    ...
    if (Get-Command -Name Get-AppxProvisionedPackage -ErrorAction SilentlyContinue) {
        try {
            Get-AppxProvisionedPackage -Online -ErrorAction SilentlyContinue |
                Where-Object { $_.DisplayName -match '(?i)dropbox' -and
    ...
    #----------------------------------------------------------------------------
    # 8. VERIFY -- identical classification rules as Detect-Dropbox.ps1 (ARP
    #    both HKLM views + per-user keys of LOADED hives + binary-gated
    ...
    foreach ($b in $DbxBinaryNames) { [void]$binarySet.Add($b) }
    ...
    function Test-LiveDropboxBinary {
        param([string]$Dir)
    ...
    foreach ($dir in ($MachineDirs | Select-Object -Unique)) {
        if (Test-LiveDropboxBinary -Dir $dir) { $residual.Add("folder: $dir") }
    }
    foreach ($prof in $profiles) {
        $profPath = $prof.ProfileImagePath
        ...
        foreach ($rel in $UserAppDirRel) {
            $dir = Join-Path $profPath $rel
            if (Test-LiveDropboxBinary -Dir $dir) { $residual.Add("user folder: $dir") }
        }
    }
    ...
    if ($residual.Count -eq 0) { Write-Log 'VERIFY: no residual Dropbox found.' }
    ...
    if ($failures.Count -gt 0) {
        Write-Log "RESULT: Dropbox removal completed WITH FAILURES." 'ERROR'
        try { Stop-Transcript -ErrorAction SilentlyContinue | Out-Null } catch { }
        Write-Output "Dropbox removal incomplete: $summary"
        exit 1
    } else {
        Write-Log 'RESULT: Dropbox removal completed successfully.'
        try { Stop-Transcript -ErrorAction SilentlyContinue | Out-Null } catch { }
        Write-Output "Dropbox removal OK: $summary"
        exit 0
    }
}
catch {
    ...
    try { Stop-Transcript -ErrorAction SilentlyContinue | Out-Null } catch { }
    Write-Output "Dropbox remediation error: $($_.Exception.Message)"
    exit 1
}
